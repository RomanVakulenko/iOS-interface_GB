...
extension GallaryViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let fullScreenView = UIView(frame: self.view.bounds) //создадим Вью
        let fullScreenImageView = UIImageView(frame: fullScreenView.frame) //надо добавить ImageView
        fullScreenView.addSubview(fullScreenImageView)// добавляем на фуллСкринВью ImageView
        ...
        self.view.addSubview(fullScreenView) // 14m добавляем на Вью
    ...
    //нам нужны будут параметры, чтобы выцепить из него Вьюшку
