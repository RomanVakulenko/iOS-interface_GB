...
extension GallaryViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let image = UIImage(named: currentFotoAlbum[indexPath.item].url) //рисует не просто Петра какого-то, а чтото из фотоальбома; то, откуда он возьмет имя файла, в котором будут лежать фотос
        
        cell.configure(image: image, isLiked: currentFotoAlbum[indexPath.item].isLiked, likeCounter: currentFotoAlbum[indexPath.item].likeCounter, onlikeClosure: { [weak self] isLikePressed, currentCounter in //8L1h38m конфигурируем дополнительно клоужер; в клоужер прилетает 2 параметра нажата/нет и числоНажатий; 8L2h00 с использованием Storage мы должны в конфигураторе не просто лайкКаунтер , а должны считать его например из фотольбома
            ...
            //8L1h57m можем в клоужере поменять наш singleton:
